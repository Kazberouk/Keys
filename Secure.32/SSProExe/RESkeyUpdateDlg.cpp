...
#define _UPDATE_DOUBLE_DATA_(mem_var1, mask1, mem_var2, mask2)														\
	case SKM_##mask1##:																									\
		KeyData.##mem_var1 = (WORD)((KeyData.##mem_var1 & ~GET_DATA(mask1)) | GET_DATA(mask1));	\
		KeyData.##mem_var2 = (WORD)((KeyData.##mem_var2 & ~GET_DATA(mask2)) | GET_DATA(mask2));	\
		break;
...
//		module_access = (eKeyUpdaterAccess)pKey->m_pModules[index+1];
		switch(pKey->m_pModules[index])
		{
			...
			_UPDATE_LEGACY_DATA_(m_MaxproVM			, VM				);
		}
		index++;
	}
#undef GET_DATA
...
void CRESkeyUpdateDlg::GetKeyInfo()
{
	...
	BOOL bNotUsedKey_Based_on_Version = TRUE; // Key.m_KeyData.m_Updater_Version < UPDATER_VERSION_NUMBER;
