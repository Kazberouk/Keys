...
void CSSPro60Dlg::DoDataExchange(CDataExchange* pDX) {
	...
	DDX_Check(pDX, IDC_STIM_RAW_DATA, m_FRAC_SCREEN);
	DDX_Check(pDX, IDC_REGISTERED, m_LOGPRO);

	DDX_Check(pDX, IDC_FIELDPRO_VM, m_VM);
...
#define _SET_DOUBLE_DATA_(mask1, mask2)	WORD(	IsAccess(m_##mask1##)*_SSP60_##mask1##_MASK_) | WORD(	IsAccess(m_##mask2##)*_SSP60_##mask1##_MASK_)
#define _SET_LEGACY_DATA_(mask1, var1)		WORD(	IsAccess(m_##var1## )*_SSP60_##mask1##_MASK_)

	KeyData.m_Fieldpro					= WORD(_SET_DATA_(FIELDPRO	));				
	KeyData.m_Fracpro						= WORD(_SET_DATA_(FRACPRO	));
	KeyData.m_FracScr_Stimpro			= WORD(_SET_DOUBLE_DATA_(FRAC_SCREEN, FRACPRO)+ _SET_DATA_(STIMPRO	));
	KeyData.m_Respro_Cementpro			= WORD(_SET_DATA_(RESPRO	)	+ _SET_DATA_(CEMENTPRO	));
	KeyData.m_MaxproVM					= WORD(_SET_DATA_(MAXPRO	)	+ _SET_DATA_(VM));
	KeyData.m_PorPro_EcoPro				= WORD(_SET_DOUBLE_DATA_(FLOPRO, WTPA)	+ _SET_DATA_(ECOPRO)	+ _SET_LEGACY_DATA_(COST, ECOPRO));
	KeyData.m_WTPA_Toolpro				= WORD(_SET_DATA_(WTPA		)	+ _SET_DATA_(TOOLPRO		)	+ _SET_LEGACY_DATA_(GRADIENT	, WTPA));
	KeyData.m_TransPro					= WORD(_SET_LEGACY_DATA_(TRANSPRO	, TOOLPRO));
	KeyData.m_eFieldpro					= WORD(_SET_LEGACY_DATA_(EFIELDPRO	, FIELDPRO));
	KeyData.m_Realpro						= WORD(_SET_DATA_(REALPRO	));
	KeyData.m_GeoPro_LogPro				= WORD(_SET_LEGACY_DATA_(GEOPRO, LOGPRO)	+ _SET_DATA_(LOGPRO	));
//	KeyData.m_Word22						= WORD(_SET_DATA_(FORECAST	));

#undef _SET_DATA_
#undef _SET_LEGACY_DATA_
...
void CSSPro60Dlg::LoadDialogFromKeyData(CSSPro60Data &KeyData) {
	...
	_GET_DATA_(m_FracScr_Stimpro			, STIMPRO		);
	_GET_DATA_(m_FracScr_Stimpro			, FRAC_SCREEN	);
	_GET_DATA_(m_Respro_Cementpro			, RESPRO			);
	_GET_DATA_(m_Respro_Cementpro			, CEMENTPRO		);
	_GET_DATA_(m_Realpro						, REALPRO		);
	_GET_DATA_(m_WTPA_Toolpro				, TOOLPRO		);
	_GET_DATA_(m_WTPA_Toolpro				, WTPA			);
	_GET_DATA_(m_MaxproVM					, MAXPRO			);
	_GET_DATA_(m_MaxproVM					, VM				);
	_GET_DATA_(m_Fieldpro					, FIELDPRO		);
	_GET_DATA_(m_PorPro_EcoPro				, FLOPRO			);
	_GET_DATA_(m_PorPro_EcoPro				, ECOPRO			);
	_GET_DATA_(m_GeoPro_LogPro				, LOGPRO			);
//	_GET_DATA_(m_WTPA_Toolpro				, GRADIENT		);
//	_GET_DATA_(m_PorPro_EcoPro				, COST			);
